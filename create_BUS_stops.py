...
stops=stops.merge(trips, on='trip_id', how='outer')# table join to get route_id; Note: multiple routes stop at the same stop=> outer join
...
local=unique_routes_at_stops['route_id'].str.match(r'\w\d+|BX\d+')# creates a booleaen mask for local routes in Bus Co
...
express_stops=express_stops.drop_duplicates(['stop_id','stop_lat','stop_lon'])## we only one point for each local route's stop
local_stops=local_stops.drop_duplicates(['stop_id','stop_lat','stop_lon'])## only one point for each express route's stop

local_stops=pd.DataFrame(local_stops, columns=['stop_id', u'stop_name', u'stop_lat', u'stop_lon'])# create df with selected columns
...
for borough in boroughs:
    ...
    local=unique_routes_at_stops['route_id'].str.match(r'[^X\.*?]')# creates a booleaen mask local routes in MTA's Bus Routes
    local_stops=unique_routes_at_stops[local]# filters out only local stops
    express_stops=unique_routes_at_stops[~local]# filters out only express stops
...
nyc_bus=gpd.GeoDataFrame(pd.concat(all_local_buses, ignore_index=True)) # merges all local services into a single gdf 
nyc_bus=gpd.GeoDataFrame(nyc_bus,columns=['stop_id','geometry','stop_name','stop_lat','stop_lon'])
nyc_bus.crs={'init' :'epsg:2263'}

express_nyc=gpd.GeoDataFrame(pd.concat(all_express_buses,ignore_index=True))# merges all express services into a single gdf  
...
nyc_bus.to_file(os.path.join(path_name,folder_name,'shapes','{}'.format(local_service)))# write local stops to shapefile
express_nyc.to_file(os.path.join(path_name,folder_name,'shapes','{}'.format(express_service)))# write expres stops to shapefile
