...
path_name=r'\\bctc-nas\LibShare\Shared\Divisions\Graduate\GEODATA\MASS_Transit' # this path is assumed to stay the same
...
for rail in rails:    
    ...
    if rail=='nyc_subway':
        shapes=shapes[~shapes.shape_id.isin(segments_to_remove)] #create new df that doesn't contain unusual service for MTA        
    ...
    if rail=='metro_north':
        trips=trips[trips.service_id==1]# filter by service id; metro_north feeds have some extra non-metro-north points
    ...
    gdf = gdf.groupby(['shape_id'])['geometry'].apply(lambda x: LineString(x.tolist())).reset_index()# create lines from points grouped by same id
    gdf_to_dis = gpd.GeoDataFrame(gdf, geometry='geometry')
    
    if rail=='nyc_subway':
        gdf_to_dis['route_id']=gdf_to_dis['shape_id'].apply(lambda x: get_route_id(x))
        gdf=gdf_to_dis.dissolve(by='route_id',as_index=False) # dissolves on common id; note:if index isn't set to False, route_id will become gdf's index
    else:
        gdf=gdf_to_dis.merge(trips, on='shape_id')# table join
        gdf=gdf.dissolve(by='route_id',as_index=False) # dissolves on common id; note:if index isn't set to False, route_id will become gdf's index
    ...
    if rail=='nyc_subway':
        ...
        gdf=gdf.drop(['shape_id','service_id'],1) ## shape_id and service id column is no longer neeed, remove it before saving to shapefile
