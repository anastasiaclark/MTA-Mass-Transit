...
local_mask = si_gdf['route_id'].str.match(
    r'[^X\.*?]')  ## regex to match any character that is not X; X indicates express service in MTA Bus routes
...
express_nyc = gpd.GeoDataFrame(pd.concat([express_si, express_qn, express_mn, express_bk, express_bx, express_gdf],
                                         ignore_index=True))  # merges all express services into a single gdf
express_nyc = gpd.GeoDataFrame(express_nyc,
                               columns=['route_id', 'dir_id', 'route_dir', 'geometry', 'route_short', 'route_long',
                                        'color'])
express_nyc.crs = {'init': 'epsg:2263'}

nyc_bus = gpd.GeoDataFrame(pd.concat([local_bk, local_bx, local_mn, local_gn, local_si, local_gdf],
                                     ignore_index=True))  # merges all local services into a single gdf
nyc_bus = gpd.GeoDataFrame(nyc_bus, columns=['route_id', 'dir_id', 'route_dir', 'geometry', 'route_short', 'route_long',
                                             'color'])
