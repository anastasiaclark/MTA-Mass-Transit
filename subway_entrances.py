...
entrances.update(entrances.loc[entrances['lon'] > 0, 'lon'].mul(
    -1))  # multiples longtitude by -1 where it is positive (in US it will always be negative)
geometry = [Point(xy) for xy in
            zip(entrances.lon, entrances.lat)]  # create a geometry object from lat, lon coordinates
...
gdf['free_cross'] = gdf['free_cross'].astype(
    str)  # boolean fields can't be written into shapefile by Fiona==> change them into strings
