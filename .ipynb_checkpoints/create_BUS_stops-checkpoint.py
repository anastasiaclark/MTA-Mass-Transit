...
local_stops = pd.DataFrame(local_stops, columns=['stop_id', 
                                                 'stop_name', 
                                                 'stop_lat',
                                                 'stop_lon'])  # create df with selected columns
local_stops.reset_index()
express_stops = pd.DataFrame(express_stops, columns=['stop_id', 
                                                     'stop_name',
                                                     'stop_lat', 
                                                     'stop_lon'])
...
for borough in boroughs:
    ...
    local = unique_routes_at_stops['route_id'].str.match(
        r'(?!^X\.*?)(?!SIM\.*?)')  # creates a booleaen mask local routes in MTA's Bus Routes
...
nyc_bus = gpd.GeoDataFrame(nyc_bus, columns=['stop_id', 
                                             'geometry', 
                                             'stop_name', 
                                             'stop_lat', 
                                             'stop_lon'])
...
express_nyc = gpd.GeoDataFrame(express_nyc, columns=['stop_id', 
                                                     'geometry', 
                                                     'stop_name', 
                                                     'stop_lat', 
                                                     'stop_lon'])
